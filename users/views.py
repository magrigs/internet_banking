...
def transactions(request):
    ...
    cursor.execute("SELECT * FROM transaction WHERE transaction_user_id ="+str(request.session.get('user_id', None))+"")
...
def transfer(request):
    ...
    if(request.method == "POST"):
        ...
        """, (request.POST['transfer_user_id'], "Credit",request.POST['transfer_amount'], request.POST['transfer_amount'] + " credited to your account", "dfsdf"))
        ...
        cursor.execute("""
            INSERT INTO `transaction`
            SET transaction_user_id=%s, transaction_type=%s, transaction_amount=%s, transaction_description=%s, transaction_date=%s   
        """, (request.session.get('user_id', None), "Debit",request.POST['transfer_amount'], request.POST['transfer_amount'] + " debited from your account", "dfsdf"))
    
        messages.add_message(request, messages.INFO, "Transfer of "+request.POST['transfer_amount']+"/- has been done succesfully to user account.")
...
def getTransactionJSON(request,one=False):
    ...
    cur.execute("SELECT transaction_amount FROM transaction WHERE transaction_user_id ="+str(request.session.get('user_id', None))+"")
...
def add(request):
    ...
    if (request.method == "POST"):
        try:
            ...
            return HttpResponse('Something went wrong. Error Message : '+ str(e))
        messages.add_message(request, messages.INFO, "Your account has been registered successfully. Login with your username and password")
